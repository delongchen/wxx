...
const configHandle = createConfigHandle<WxxAppConfig>(
  'app',
  'app.config',
)
...
export const fetchLocalConfig = (): AppThunk =>
  async (dispatch) => {
    const localConfig = await configHandle
      .readWithInit(initialWxxAppConfig)
      .catch(() => null)
...
export const setGlobalThemeAsync = (theme: string): AppThunk =>
  async (dispatch) => {
