...
fn merge_object(target: AnyObject, source: AnyObject) -> AnyObject {
...
pub async fn read_config<R: Runtime>(
    ...
    let file_path = resolve_config_file(data_path, namespace, config_name, config_type);
    ...
    let config_data =
        serde_json::from_str::<AnyObject>(&config_text).map_err(|_| "parse error".to_string())?;
...
pub async fn write_config<R: Runtime>(
    ...
    let mut data = match data {
        ...
        _ => return Err("data is not object".to_string()),
    };
    ...
    if file_path.exists() {
        ...
        let prev_data = serde_json::from_str::<AnyObject>(&config_text)
    ...
    let json_text = to_string_pretty(&data).map_err(|_| "json to string error".to_string())?;
